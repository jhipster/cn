...
2. 一旦`npm install`完成，它将在`package.json`中调用`postInstall`脚本，此步骤将触发`webapp:build`任务。
...
如果您在没有运行`npm start`或`yarn start`的情况下开始对前端代码进行更改，则不会反映任何内容，因为更改未被编译，因此您需要在更改后手动运行`npm run webapp:build`或运行`npm start`或`yarn start`运行。

您还可以通过开始时传递`./mvnw -Pdev，webapp`这样的`webapp`配置文件，强制Maven在运行时运行`webapp：dev`任务。
...
- 要在浏览器中处理代码，我们建议使用[Angular Augury](https://augury.angular.io/)，以便您可以查看路由并调试代码

## 项目结构

可以在`src/main/webapp`下找到JHipster前端代码，该代码与[Angular样式指南](https://angular.io/guide/styleguide)密切相关。如果您对我们的应用程序结构，文件名，TypeScript约定有任何疑问，请先阅读本指南。
...
Angular CLI用于构建和测试JHipster应用程序。
但是，我们添加了一个自定义的webpack配置文件，以通过添加BrowserSync，ESLint（Angular CLI仍在TSLint上），合并JSON转换文件并在构建完成或失败时添加通知来改善开发人员的体验。
...
您可以使用`ng build`来构建前端，但是我们仍然建议使用提供的NPM脚本，例如`npm start`，`npm run build`等。查看我们的[开发使用文档]({{ site.url }}/development/)和[生产使用文档]({{ site.url }}/production/)。

### 生成组件，指令，管道和服务

您可以使用`ng generate`（或`ng g`）命令生成Angular组件：
...
为了考虑JHipster应用程序上的一致性，可以通过`npm`命令执行测试：
...
JHipster使用`ngx-translate`依赖项进行翻译。Angular CLI i18n基于默认的Angular i18n支持，该支持与JHipster不兼容。
